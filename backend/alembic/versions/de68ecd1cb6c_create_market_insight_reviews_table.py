...
down_revision: str | Sequence[str] | None = "47feaf7f4a7d"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["market_insight_id"], ["insights.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(["reviewer_id"], ["users.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        op.f("ix_market_insight_reviews_id"),
        "market_insight_reviews",
        ["id"],
        unique=False,
    )
...
def downgrade() -> None:
    """Downgrade schema."""
    op.drop_index(
        op.f("ix_market_insight_reviews_id"), table_name="market_insight_reviews"
    )
