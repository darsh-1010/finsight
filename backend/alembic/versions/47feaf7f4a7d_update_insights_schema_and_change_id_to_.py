...
down_revision: str | Sequence[str] | None = "dbbda685fdbb"
...
def upgrade() -> None:
    ...
    insight_status_enum = sa.Enum(
        "draft", "approved", "published", "archived", name="insightstatus"
    )
    ...
    op.add_column(
        "insights", sa.Column("verification_status", sa.String(), nullable=True)
    )
    op.add_column(
        "insights", sa.Column("citations", sa.ARRAY(sa.String()), nullable=True)
    )
    op.add_column("insights", sa.Column("alert_message", sa.Text(), nullable=True))
    op.add_column(
        "insights",
        sa.Column(
            "status", insight_status_enum, nullable=False, server_default="draft"
        ),
    )
