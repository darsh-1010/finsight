...
down_revision: str | Sequence[str] | None = "57c7a26720e3"
...
def upgrade() -> None:
    ...
    disclosure_type_enum = postgresql.ENUM(
        "risk", "info", "warning", "success", name="disclosuretype"
    )
    ...
    op.create_index(
        op.f("ix_compliance_groups_id"), "compliance_groups", ["id"], unique=False
    )
    op.create_index(
        op.f("ix_compliance_groups_key"), "compliance_groups", ["key"], unique=True
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["group_id"],
            ["compliance_groups.id"],
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        op.f("ix_compliance_disclosures_id"),
        "compliance_disclosures",
        ["id"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_compliance_disclosures_id"),
        table_name="compliance_disclosures",
        if_exists=True,
    )
    ...
    op.drop_index(
        op.f("ix_compliance_groups_key"), table_name="compliance_groups", if_exists=True
    )
    op.drop_index(
        op.f("ix_compliance_groups_id"), table_name="compliance_groups", if_exists=True
    )
