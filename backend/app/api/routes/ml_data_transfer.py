...
def verify_ml_token(
    x_ml_token: str = Header(..., description="Secret token from ML server"),
):
...
def create_ml_sub_urls(
    request: list[ScrapingSubURLCreate], db: Session = Depends(deps.get_db)
):
    try:
...
def delete_ml_sub_urls(
    request: DeleteDocumentsRequest, db: Session = Depends(deps.get_db)
):
    try:
        deleted_count = (
            db.query(ScrapingSubURL)
            .filter(ScrapingSubURL.document_id.in_(request.document_ids))
