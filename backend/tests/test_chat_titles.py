...
from app.models.chat import ChatSession, ChatMessage, UsageCounter
...
def db():
    ...
    tier = Tier(id=1, name="Foundation", level=1, description="Free tier", price_amount=0)
    db.add(user_role)
    db.add(tier)
    db.commit()
...
def test_chat_session_title(client, db):
    ...
    found_session = next((s for s in all_sessions if s["session_id"] == session_id), None)
...
def test_chat_session_with_custom_title(client, db):
    ...
    assert final_session.get("title") == custom_title  # Should still be the custom title
