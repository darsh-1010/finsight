"""Unit tests for the Daily Summary Compiler service.

Tests cache retrieval, dynamic yFinance integration,
...
from src.services.market_insights.daily_models import (DailyStockHighlight,
                                                       DailySummaryReport)
...
def get_sample_raw_alerts() -> list[AlertPayload]:
    ...
    insight_1 = InsightResult(
        ...
        confidence=0.92,
        summary="Apple drops 2.4% intraday.",
    )
...
def get_sample_report() -> DailySummaryReport:
    """Generates a sample target DailySummaryReport."""
    highlight_aapl = DailyStockHighlight(
        ticker="AAPL",
        category=InsightCategory.PRICE_ACTION,
        topic=InsightTopic.INTRADAY_DROP,
        daily_trend="Bearish",
        ...
        alert_message="📉 Apple dips -2.4% — Stock falls on intraday sell-off.",
        insight_source="yfinance",
    )

    highlight_tsla = DailyStockHighlight(
        ticker="TSLA",
        category=InsightCategory.PRICE_ACTION,
        topic=InsightTopic.VOLUME_SPIKE,
        daily_trend="Bullish",
        ...
        alert_message="📈 Tesla rises +1.2% — Volume surges.",
...
def get_mock_download_data(tickers: list[str]) -> pd.DataFrame:
    """Generates 20 rows of mock Close and Volume data for list of tickers."""
    columns = pd.MultiIndex.from_product([tickers, ["Close", "Volume"]])
    data_dict = {}
    for ticker in tickers:
        data_dict[(ticker, "Close")] = [180.0] * 20
        data_dict[(ticker, "Volume")] = [100000] * 20
...
async def test_compiler_cache_hit() -> None:
    ...
    compiler = DailySummaryCompiler(redis_client=mock_redis)
    ...
    mock_redis.get.assert_called_once_with("mi:daily_report:tier:2")
...
@pytest.mark.anyio
@patch("src.services.market_insights.daily_compiler.yf.Ticker")
@patch("src.services.market_insights.daily_compiler.yf.download")
async def test_compiler_compilation_success(
    ...
    compiler = DailySummaryCompiler(openai_client=mock_openai_client, redis_client=mock_redis)

    # Mock yFinance download MultiIndex DataFrame
    mock_yf_download.return_value = get_mock_download_data(getattr(compiler, "_SCANNING_UNIVERSE"))
    ...
    assert cache_key == "mi:daily_report:tier:2"
...
async def test_compiler_instant_fallback_on_cache_miss() -> None:
    ...
    assert report.highlights[0].verification_status == "Unverified"
    assert report.highlights[0].alert_message == "AAPL — Daily market tracking update."
...
async def test_compiler_concurrency_lock_acquired() -> None:
    ...
    mock_redis.set.assert_called_once_with("mi:daily_report:lock:tier:2", "locked", ex=180, nx=True)
