...
class FallbackAsyncOpenAI:
    ...
    def __init__(self, api_key: str | None = None) -> None:
        ...
        self._primary_client = AsyncOpenAI(
            api_key=primary_key, base_url=settings.freellmapi_base_url
        )

        fallback_key = api_key or settings.openai_api_key
        self._fallback_client = AsyncOpenAI(api_key=fallback_key)

        self.chat = self.ChatWrapper(self._primary_client, self._fallback_client)
        self.embeddings = self.EmbeddingsWrapper(
            self._primary_client, self._fallback_client
        )
        ...
        class BetaChatWrapper:
            ...
            class BetaChatCompletionsWrapper:
                ...
                async def parse(self, *args: Any, **kwargs: Any) -> Any:
                    try:
                        return await self.primary.beta.chat.completions.parse(
                            *args, **kwargs
                        )
                    except Exception as e:
                        logger.warning(
                            f"Primary FreeLLMAPI failed for beta.chat.completions.parse: {e}. Falling back to OpenAI."
                        )
                        return await self.fallback.beta.chat.completions.parse(
                            *args, **kwargs
                        )
        ...
        class CompletionsWrapper:
            ...
            async def create(self, *args: Any, **kwargs: Any) -> ChatCompletion | Any:
                try:
                    ...
                    logger.warning(
                        f"Primary FreeLLMAPI failed for chat.completions.create: {e}. Falling back to OpenAI."
                    )
    ...
    class EmbeddingsWrapper:
        ...
        async def create(self, *args: Any, **kwargs: Any) -> Any:
            try:
                ...
                logger.warning(
                    f"Primary FreeLLMAPI failed for embeddings.create: {e}. Falling back to OpenAI."
                )
